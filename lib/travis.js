...
module.exports = function(cb) {
  console.log('Getting travis ready for prime time.');
  var getSlug = function(fin) {
    var pwd = process.cwd();
    var pkg = require(pwd + '/package.json');
    if (pkg.repository.url.length <= 0) {
      return fin('Cannot detect travis slug without proper package.json information.');
    }

    var url = pkg.repository.url;
    var slug = url.substring(url.indexOf('github.com/')+11, url.indexOf('.git'));
